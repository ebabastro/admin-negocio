        ...
        <ul>
            ...
            >
                <div class="border-2 flex items-center justify-between">
                    <p @click="categoryId = category.id" class="cursor-pointer">
                        {{ category.name }}
                        <font-awesome-icon
                    ...
                    >
                        <div class="flex items-center justify-between">
                            ...
                            >
                                <input
                                    type="hidden"
                                    v-model="toDeleteSubcategory.id"
                                />
                                <button>
                                    <font-awesome-icon
                                        class="text-xs mr-2 text-red-700"
            ...
            <li>
                ...
                <form v-else @submit="storeCategory" action="#">
                    <div class="flex items-center space-x-1">
                        <input
                ...
                <span class="bg-red-600 text-slate-50" v-show="error.error">{{
                    error.message
                }}</span>
            </li>
        </ul>
...
function storeCategory(ev) {
    ev.preventDefault();
    store
        .dispatch("storeCategory", newCategory)
        .then(() => {
...
function storeSubcategory(fk_category_id, ev) {
    ...
    store
        .dispatch("storeSubcategory", newSubcategory)
        .then(() => {
...
function deleteCategory(categoryId) {
    ...
    store.dispatch("deleteCategory", toDeleteCategory.value).then(() => {
        store.dispatch("getCategories");
    });
}
...
function deleteSubcategory(subcategoryId) {
    ...
    store.dispatch("deleteSubcategory", toDeleteSubcategory.value).then(() => {
        store.dispatch("getSubcategories");
    });
}
</script>
