<template>
	<div class="bg-cyan-600 text-cyan-50 transition-all duration-700 relative flex flex-col items-center"
		:class="collapsed ? 'w-16' : 'w-64'">
		<!-- Se muestra cuando esta colapsada -->
		<div class="h-20 pt-5 bg-cyan-700 w-full shadow text-center font-semibold">

			<span class="flex items-center justify-center space-x-5">
				<font-awesome-icon icon="fa-solid fa-mug-saucer" class="w-10 h-10"/>
				<h1 class="text-2xl" :class="[!collapsed ? '' : 'hidden']">SIDEBAR</h1>
			</span>
		</div>
		...
		<span class="absolute bottom-0 left-0 transition duration-700 hover:animate-pulse hover:scale-125 hover:cursor-pointer"
...
<script setup>
	...
	const navigation = [
		...
		{ name: "Almacen", to: { name: "almacenes" }, icon: "fa-solid fa-box-open" },
	];
